...
- (void)setFrame:(CGRect)newFrame {
    ...
    CGFloat posY = ceil((self.segmentedControl.bounds.size.height-self.font.pointSize+self.font.descender)/2)+self.segmentedControl.titleEdgeInsets.top-self.segmentedControl.titleEdgeInsets.bottom-self.segmentedControl.thumbEdgeInset.top+1;
	int pointSize = self.font.pointSize;
	
	if(pointSize%2 != 0)
		posY--;
    
    // make it to bold explict here, since it is wired not bold if didn't set here
    self.label.font = [UIFont boldSystemFontOfSize:self.font.pointSize];
