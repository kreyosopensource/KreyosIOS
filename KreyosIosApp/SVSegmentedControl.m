...
@synthesize dividerColor, bottomColor;
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
	...
	self.segmentWidth = self.frame.size.width / c;
...
//	self.segmentWidth = ceil(self.segmentWidth/2.0)*2; // make it an even number so we can position with center
    ...
    self.thumbHeight = self.thumb.backgroundImage ? self.thumb.backgroundImage.size.height : self.frame.size.height-(self.thumbEdgeInset.top+self.thumbEdgeInset.bottom);
...
- (void)drawRect:(CGRect)rect {
    ...
    else
    {
        ...
        if (dividerColor)
        {
            ...
            float titleWidth = rect.size.width / [self.titlesArray count];
            for (int i = 1; i < [self.titlesArray count]; ++i)
            {
                CGContextFillRect(context, CGRectMake(titleWidth*i, 0, lineWidth, rect.size.height));
            }
        }
    }
//    else {
...
//        //CGFloat components[4] = {0.1, 0.1,  0.1, 1};
...
//    }
	...
	for(id title in self.titlesArray) {
        ...
        if([title isKindOfClass:[UIImage class]])
        {
            ...
            CGRect labelRect = CGRectMake((self.segmentWidth*i), 2, self.segmentWidth,self.frame.size.height - 4);
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    if (self.snapToIndex == self.selectedIndex) {
        return NO;
    }
